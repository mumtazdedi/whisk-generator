...
class WhiskTerminalApp {
  ...
  async autoLoadSettings() {
    try {
      ...
      console.log(chalk.green(`✓ Settings auto-loaded from ${this.settingsFile}`));
    } catch (error) {
      console.log(chalk.yellow(`⚠️  No settings found, using defaults. Will create ${this.settingsFile} on first save.`));
    }
  }
  ...
  async autoSaveSettings() {
    try {
      ...
      console.log(chalk.red(`❌ Failed to auto-save settings: ${error.message}`));
    }
  }

  initializeClients() {
    this.clients = this.config.tokens.map(token => new WhiskClient(token));
    this.currentTokenIndex = 0;
  }
  ...
  async loadPromptsFromFile(filePath) {
    try {
      ...
      console.log(
        chalk.green(`✓ Successfully loaded ${prompts.length} prompts from ${filePath}`),
      );
      return prompts;
    } catch (error) {
  ...
  displayMainMenu() {
    ...
    const tokenStatus = tokenCount > 0 
  ...
  async showTokenMenu() {
    ...
    const tokenCount = this.config.tokens.length;
    if (tokenCount > 0) {
      console.log(chalk.green(`Currently configured: ${tokenCount} token(s)\n`));
    } else {
      console.log(chalk.yellow("⚠️  No tokens configured - required for image generation!\n"));
    }
  ...
  async addToken() {
    console.clear();
    console.log(chalk.blue.bold("\n➕ Add New Token\n"));
    console.log(chalk.yellow("💡 Tip: You can add multiple tokens for faster generation with multiple workers."));
    ...
    console.log(chalk.green(`✅ Token added successfully! Total tokens: ${this.config.tokens.length}`));
    console.log(chalk.blue(`💡 You can now use ${this.config.tokens.length} worker(s) for parallel generation.`));
  ...
  async viewTokens() {
    ...
    if (this.config.tokens.length === 0) {
      ...
      console.log(chalk.green(`Total configured tokens: ${this.config.tokens.length}\n`));
      this.config.tokens.forEach((token, index) => {
        ...
        console.log(`${chalk.cyan(`${index + 1}.`)} ${masked}`);
      });
  ...
  async removeToken() {
    ...
    if (isNaN(tokenIndex) || tokenIndex < 0 || tokenIndex >= this.config.tokens.length) {
  ...
  async clearAllTokens() {
    ...
    console.log(chalk.yellow(`⚠️  This will remove all ${this.config.tokens.length} configured tokens.`));
  ...
  async configureOutputDirectory() {
    ...
    try {
      ...
      const imageFiles = files.filter(f => f.toLowerCase().match(/\.(jpg|jpeg|png|webp)$/));
      console.log(chalk.green(`✓ Directory exists (${imageFiles.length} image files found)`));
    } catch {
      console.log(chalk.yellow("⚠️  Directory does not exist (will be created automatically)"));
    }
  ...
  async configureAspectRatio() {
    ...
    if (choiceNum === 0) {
      ...
      console.log(chalk.green("✅ Set to random aspect ratio! Each image will use a different aspect ratio."));
    } else {
      ...
      console.log(chalk.green(`✅ Aspect ratio set to: ${aspectOptions[selectedKey].display}`));
    }
  ...
  async configureCompression() {
    ...
    console.log(chalk.yellow("💡 Higher quality = larger file size, lower quality = smaller file size"));
    ...
    if (isNaN(compressionNum) || compressionNum < 1 || compressionNum > 100) {
      console.log(chalk.red("❌ Invalid quality value. Please enter a number between 1-100."));
      await this.promptUser("⏎ Press Enter to continue...");
      return;
    }
    ...
    console.log(chalk.blue(`Changed from: ${oldQuality}% to ${compressionNum}%`));
    
    if (compressionNum >= 95) {
      console.log(chalk.yellow("💡 Very high quality - expect larger file sizes"));
    } else if (compressionNum <= 50) {
      console.log(chalk.yellow("💡 Low quality - files will be small but may show compression artifacts"));
    }
  ...
  async configureWorkers() {
    ...
    console.log(chalk.cyan(`Available tokens: ${this.config.tokens.length}`));
    
    if (this.config.tokens.length === 0) {
      console.log(chalk.yellow("⚠️  No tokens configured! Add tokens first before setting workers."));
      await this.promptUser("⏎ Press Enter to continue...");
      return;
    }

    console.log(chalk.yellow(`💡 Recommended: Use up to ${this.config.tokens.length} worker(s) (one per token)`));
    console.log(chalk.gray("   More workers = faster generation (if you have multiple tokens)"));
    ...
    if (isNaN(workersNum) || workersNum < 1 || workersNum > 10) {
      console.log(chalk.red("❌ Invalid workers count. Please enter a number between 1-10."));
      await this.promptUser("⏎ Press Enter to continue...");
      return;
    }
    ...
    console.log(chalk.blue(`Changed from: ${oldWorkers} to ${workersNum} worker(s)`));
    
    if (workersNum > this.config.tokens.length) {
      console.log(chalk.yellow(`⚠️  You have more workers (${workersNum}) than tokens (${this.config.tokens.length})`));
      console.log(chalk.yellow("   Workers will share tokens, which may be slower"));
    }
  ...
  async setPromptFile() {
    ...
    if (this.config.promptFile) {
      ...
      try {
        ...
        const prompts = content
          ...
          .map(line => line.trim())
          .filter(line => line.length > 0 && !line.startsWith("#"));
        console.log(chalk.green(`✓ File exists (${prompts.length} prompts found)`));
      } catch {
    ...
    console.log(chalk.gray("   Completed prompts will be automatically removed from the file"));
    ...
    try {
      ...
      if (prompts.length === 0) {
        console.log(chalk.yellow("⚠️  File is empty or contains no valid prompts"));
      }
    } catch (error) {
  ...
  async viewStatus() {
    ...
    // Tokens
    const tokenStatus = this.config.tokens.length > 0 
    ...
    try {
      await fs.access(this.config.outputDir);
      const files = await fs.readdir(this.config.outputDir);
      imageCount = files.filter(f => f.toLowerCase().match(/\.(jpg|jpeg|png|webp)$/)).length;
    ...
    // Ready to generate?
    const canGenerate = this.config.tokens.length > 0 && this.config.promptFile && promptCount > 0;
    console.log(chalk.cyan.bold("\n🚀 Generation Readiness:"));
    console.log(chalk.gray("━".repeat(50)));
    
    if (canGenerate) {
      console.log(chalk.green.bold("✅ Ready to generate images!"));
      console.log(chalk.blue(`🎯 Will process ${promptCount} prompts using ${this.config.workers} worker(s)`));
      
      if (this.config.workers > this.config.tokens.length) {
        console.log(chalk.yellow(`⚠️  More workers than tokens - some workers will share tokens`));
      }
    } else {
  ...
  async generateImagesWorker(prompts, workerId, totalWorkers) {
    ...
    for (let i = 0; i < prompts.length; i++) {
      ...
      const spinner = ora(
        `Worker ${workerId + 1}: [${i + 1}/${prompts.length}] ${aspectDisplay} - "${prompt.substring(0, 40)}${prompt.length > 40 ? '...' : ''}"`,
      ).start();
      ...
      while (!success && attempts < maxAttempts) {
        ...
        if (!client) {
          spinner.fail(`Worker ${workerId + 1}: No valid tokens available`);
          results.failed++;
          break;
        }
        ...
        try {
          ...
          for (const panel of imagePanel) {
            for (const image of panel.generatedImages || []) {
              ...
              const fileName = `w${workerId + 1}_${timestamp}_${aspectShort}_${savedCount + 1}.jpg`;
  ...
  async generateImages() {
    ...
    if (this.config.tokens.length === 0) {
      ...
      console.log(chalk.blue("💡 Go to: Main Menu → 1. Manage API Tokens → 1. Add New Token"));
      await this.promptUser("⏎ Press Enter to continue...");
      return;
    }

    if (currentPrompts.length === 0) {
      console.log(chalk.red("❌ No prompts available! Please set a prompt file with valid prompts."));
    ...
    if (this.config.workers > this.config.tokens.length) {
      console.log(chalk.yellow(`⚠️  You have more workers (${this.config.workers}) than tokens (${this.config.tokens.length})`));
      console.log(chalk.yellow("   Some workers will share tokens, which may slow down generation"));
    }

    if (this.config.randomAspectRatio) {
      console.log(chalk.magenta("🎲 Random aspect ratio is enabled - each image will have a different aspect ratio"));
    }
    ...
    const workerPromises = promptsPerWorker.map((prompts, workerId) => {
      if (prompts.length === 0)
        return Promise.resolve({ success: 0, failed: 0, images: [], completedPrompts: [] });
    ...
    try {
      ...
      if (totalFailed > 0) {
        console.log(chalk.red(`❌ Failed prompts: ${totalFailed}`));
      }
      ...
      console.log(chalk.gray(`🔧 Used ${this.config.workers} worker(s) with ${this.config.tokens.length} token(s)`));
  ...
  async run() {
    ...
    console.log(chalk.yellow("💡 First time? Set up your API tokens and prompt file to get started.\n"));
    ...
    while (true) {
      ...
      switch (choice) {
        ...
        case "5":
          ...
          break;
        case "6":
          console.log(chalk.blue("👋 Thank you for using Whisk AI Image Generator!"));
